				...
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						...
						<span className="text-sm text-muted-foreground">
							{selectedReportForms.length} dipilih
						</span>
					</div>

					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{forms.map((form) => {
							const isSelected = selectedReportForms.some(
								(f) => f._id === form._id,
							);
							return (
								<div
								...
								<Card
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start mt-6 pt-6 border-t">
										...
										<RoleAccessSelector
											...
											onToggleRole={(role) =>
												toggleRoleFill(form._id, role, "report")
											}
											positions={activePositions}
											selectedPositionIds={config.fillableByPositionIds}
											onTogglePosition={(posId) =>
												toggleFillablePosition(form._id, posId, "report")
											}
										/>
										...
										<RoleAccessSelector
											...
											onToggleRole={(role) =>
												toggleRoleView(form._id, role, "report")
											}
											positions={activePositions}
											selectedPositionIds={config.viewableByPositionIds}
											onTogglePosition={(posId) =>
												toggleViewablePosition(form._id, posId, "report")
											}
										/>
									</div>
