...
import {
	FormBuilderEdit,
	type FormBuilderEditRef,
} from "../components/create/form-builder-edit";
...
const FormEditPage: React.FC = () => {
	...
	const formRef = useRef<FormBuilderEditRef>(null);
	const [fields, setFields] = useState<Field[]>([]);
	const [isSubmitting, setIsSubmitting] = useState(false);

	if (loading) return <SectionLoading message="Memuat formulir..." />;
	if (!detailForm) return null;

	const initialData: CreateFormRequest = {
	...
	return (
		<>
			...
			<PageHeader
				title={`Edit Formulir`}
				subtitle={`Perbarui detail dan pertanyaan formulir "${detailForm.title}".`}
				backPath={true}
			/>

			{/* Main Layout */}
			<div className="flex flex-col lg:flex-row items-start gap-6">
				{/* Form Builder Edit — full width mobile, flex-1 desktop */}
