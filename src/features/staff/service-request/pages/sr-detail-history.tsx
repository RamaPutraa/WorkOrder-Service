...
import WorkReportDrawer from "@/features/client/services-wo/components/work-report-drawer";
...
const SrDetailHistory = () => {
	...
	const fetchDetail = async () => {
		...
		const { data: reportRes } = await handleApi(() => getStaffWorkReport(id));
		if (reportRes?.data) {
	...
	const handleSubmitReview = async () => {
		...
		const { error } = await handleApi(() =>
			submitStaffReviewApi(id, submissionPayload),
		);
